...
# check trait for gwaa object

check_trait_gwaa <- function(trait, gwaa){

  # get the phenotypic values data.frame
  ...
  if(is.numeric(trait)){
    ...
    nb.trait <- dim(pheno)[2]

    if(!((0 < trait) && (trait <=nb.trait))){

      stop(paste("The trait indicator should be between 1 and", nb.trait,
                 "the total number of traits in the gwaa object"))
